...
{
    public static readonly Guid ServiceTypeId = Guid.Parse("5be26920-84c4-4f03-bb02-6ef1786e5ac0");

    private const string InstanceIdFileName = "cartographicprojection-mcp-hub-instance-id.txt";
        ...
        while (!stoppingToken.IsCancellationRequested)
        {
            ...
            try
            {
                ...
                if (response.IsSuccessStatusCode)
                {
                    ...
                    _logger.LogInformation("Registered CartographicProjection MCP endpoint on MCP hub at {HubUri}", collectionUri);
                }
                else
                {
    ...
    private static McpHubRegistration CreateRegistration(McpHubOptions options, Guid instanceId)
    {
        ...
        string mcpHttpUrl = $"{publicBaseUrl}/CartographicProjection/api/mcp";
        string mcpWebSocketUrl = ToWebSocketUrl($"{publicBaseUrl}/CartographicProjection/api/mcp/ws");

        return new McpHubRegistration(
            ServiceTypeId,
            instanceId,
            string.IsNullOrWhiteSpace(options.ServiceName) ? "CartographicProjection" : options.ServiceName,
            mcpHttpUrl,
            mcpWebSocketUrl,
