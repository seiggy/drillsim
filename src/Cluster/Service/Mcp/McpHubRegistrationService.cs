...
{
    public static readonly Guid ServiceTypeId = Guid.Parse("f0a90d5f-dc55-4e15-9d1a-2467a971cd74");

    private const string InstanceIdFileName = "cluster-mcp-hub-instance-id.txt";
        ...
        while (!stoppingToken.IsCancellationRequested)
        {
            ...
            try
            {
                ...
                if (response.IsSuccessStatusCode)
                {
                    ...
                    _logger.LogInformation("Registered Cluster MCP endpoint on MCP hub at {HubUri}", collectionUri);
                }
                else
                {
    ...
    private static McpHubRegistration CreateRegistration(McpHubOptions options, Guid instanceId)
    {
        ...
        string mcpHttpUrl = $"{publicBaseUrl}/Cluster/api/mcp";
        string mcpWebSocketUrl = ToWebSocketUrl($"{publicBaseUrl}/Cluster/api/mcp/ws");

        return new McpHubRegistration(
            ServiceTypeId,
            instanceId,
            string.IsNullOrWhiteSpace(options.ServiceName) ? "Cluster" : options.ServiceName,
            mcpHttpUrl,
            mcpWebSocketUrl,
