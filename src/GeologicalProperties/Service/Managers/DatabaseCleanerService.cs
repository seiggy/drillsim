...
namespace GeologicalProperties.Service.Managers
{
    public class DatabaseCleanerService : BackgroundService
    {
        ...
        private readonly string[] _dataTables = ["GeologicalPropertiesTable"];
        ...
        /// </summary>
        /// <returns>true if older Permeability were successfully deleted</returns>
        private bool CleanDatabase(string dataTable)
        {
            ...
            if (connection != null)
            {
                ...
                {
                    _logger.LogError(ex, "Impossible to clean old Permeability from {table}", dataTable);
                }
            }
            else
            {
