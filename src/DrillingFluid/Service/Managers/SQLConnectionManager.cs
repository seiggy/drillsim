...
namespace NORCE.Drilling.DrillingFluid.Service.Managers
{
    ...
    public class SqlConnectionManager
    {
        ...
        public static readonly string DATABASE_FILENAME = "DrillingFluid.db";
        ...
        // 1) to avoid loading the complete DrillingFluid (heavy weight data) each time we only need contextual info on the data (light weight data)
        // 2) to keep control of the logic of inserting and selecting a light data in the database
        //    localized at the controller/manager level (storing DrillingFluidLight as a whole could induce database corruption issues)
        // If the light weight data concept is not implemented, the same contextual info can be retrieved directly from the DrillingFluid
        private readonly static Dictionary<string, string[]> _tableStructureDict = new Dictionary<string, string[]>()
            {
                { "BaseOilTable", new string[] {
                    "ID text primary key",
                    "MetaInfo text",
                    "BaseOil text" }
                },
                { "BrineTable", new string[] {
                    "ID text primary key",
                    "MetaInfo text",
                    "Brine text" }
                },
                { "DrillingFluidDescriptionTable", new string[] {
                    "ID text primary key",
                    "MetaInfo text",
                    "DrillingFluidDescription text" }
                },
                { "DrillingFluidTable", new string[] {
                    "ID text primary key",
                    "MetaInfo text",
                ...
                { "DrillingFluidOrderTable", new string[] {
                    "ID text primary key",
                    "MetaInfo text",
                    // beginning of list of fields used only when light weight concept is implemented
                    "Name text",
