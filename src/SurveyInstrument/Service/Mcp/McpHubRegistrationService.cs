...
public sealed class McpHubRegistrationService : BackgroundService
{
    ...
    private const string InstanceIdFileName = "survey-instrument-mcp-hub-instance-id.txt";
        ...
        while (!stoppingToken.IsCancellationRequested)
        {
            ...
            try
            {
                ...
                if (response.IsSuccessStatusCode)
                {
                    ...
                    _logger.LogInformation("Registered SurveyInstrument MCP endpoint on MCP hub at {HubUri}", collectionUri);
                }
                else
                {
    ...
    private static McpHubRegistration CreateRegistration(McpHubOptions options, Guid instanceId)
    {
        ...
        string mcpHttpUrl = $"{publicBaseUrl}/SurveyInstrument/api/mcp";
        string mcpWebSocketUrl = ToWebSocketUrl($"{publicBaseUrl}/SurveyInstrument/api/mcp/ws");

        return new McpHubRegistration(
            ServiceTypeId,
            instanceId,
            string.IsNullOrWhiteSpace(options.ServiceName) ? "SurveyInstrument" : options.ServiceName,
            mcpHttpUrl,
            mcpWebSocketUrl,
