...
namespace NORCE.Drilling.GeothermalProperties.Service.Managers
{
    public class DatabaseCleanerService : BackgroundService
    {
        ...
        private readonly string[] _dataTables = ["GeothermalPropertiesCompletionOrderTable"];
        ...
        /// </summary>
        /// <returns>true if older GeothermalProperties were successfully deleted</returns>
        private bool CleanDatabase(string dataTable)
        {
            ...
            if (connection != null)
            {
                ...
                {
                    _logger.LogError(ex, "Impossible to clean old GeothermalProperties from {table}", dataTable);
                }
            }
            else
            {
