...
namespace NORCE.Drilling.GeothermalProperties.Service.Managers
{
    ...
    public class SqlConnectionManager
    {
        ...
        public static readonly string DATABASE_FILENAME = "GeothermalProperties.db";
        ...
        // 1) to avoid loading the complete GeothermalPropertiesCompletionOrder (heavy weight data) each time we only need contextual info on the data (light weight data)
        // 2) to keep control of the logic of inserting and selecting a light data in the database
        //    localized at the controller/manager level (storing GeothermalPropertiesCompletionOrderLight as a whole could induce database corruption issues)
        // If the light weight data concept is not implemented, the same contextual info can be retrieved directly from the GeothermalPropertiesCompletionOrder
        private readonly static Dictionary<string, string[]> _tableStructureDict = new Dictionary<string, string[]>()
            {
                { "GeothermalPropertiesTable", new string[] {
                    "ID text primary key",
                    "MetaInfo text",
                    "GeothermalProperties text" }
                },
                { "GeothermalPropertiesCompletionOrderTable", new string[] {
                    "ID text primary key",                    
                    "MetaInfo text",                    
                    // beginning of list of fields used only when light weight concept is implemented
                    "GeothermalPropertiesCompletionOrderLight text",
                    // end of list of fields used only when light weight concept is implemented
                    "CreationDate text",
                    "LastModificationDate text",
                    "GeothermalPropertiesCompletionOrder text" }
                }
            };
