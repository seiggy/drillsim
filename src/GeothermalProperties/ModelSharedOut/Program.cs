...
///             a json formatted OpenApi document JSON_BUNDLE is generated into the Service project so that it can be exposed publicly with Scalar
...
class Program
{
    ...
    private static readonly string JSON_BUNDLE = "GeothermalPropertiesMergedModel.json";
    private static readonly string CSHARP_MODEL = "GeothermalPropertiesMergedModel.cs";
    private static readonly string PSEUDO_CTOR = "PseudoConstructors.cs";

    // ### BEGIN CODE SPECIFIC TO ModelSharedOut 3/3 ###
    private static readonly string NAMESPACE = "NORCE.Drilling.GeothermalProperties.ModelShared"; // should be the same as for ModelSharedIn to avoid type name collision
        ...
        if (res == "Y")
        {
            try
            {
                if (Directory.Exists(jsonInputsDirectory))
                {
                    ...
                    // storing merged OpenApi document into Service to be exposed through Scalar
    ...
    static void DynamicCreationOfPseudoConstructors(DirectoryInfo directory)
    {
        ...
        if (res == "Y")
        {
            try
            {
                ...
                string namespacePath = directory.FullName + Path.DirectorySeparatorChar + MODELSHARED_FOLDER + Path.DirectorySeparatorChar + "GeothermalPropertiesMergedModel.cs";
                ...
                //Find the used class
                var type = assembly.GetType("NORCE.Drilling.GeothermalProperties.PseudoConstructorsWriter.Writer");
