...
namespace NORCE.Drilling.GeodeticDatum.Service.Managers
{
    public class DatabaseCleanerService : BackgroundService
    {
        ...
        private readonly string[] _dataTables = ["GeodeticConversionSetTable"];
        private static readonly TimeSpan _cleaningInterval = TimeSpan.FromHours(1);
        ...
        /// </summary>
        /// <returns>true if older DrillStringSection were successfully deleted</returns>
        private bool CleanDatabase(string dataTable)
        {
            ...
            if (connection != null)
            {
                ...
                {
                    _logger.LogError(ex, "Impossible to clean old DrillStringSection from {table}", dataTable);
                }
            }
            else
            {
