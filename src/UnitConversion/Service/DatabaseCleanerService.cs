...
namespace OSDC.UnitConversion.Service
{
    public class DatabaseCleanerService : BackgroundService
    {
        ...
        private readonly string[] _dataTables = ["UnitSystemConversionSetTable", "UnitConversionSetTable"];
        ...
        /// </summary>
        /// <returns>true if older data were successfully deleted</returns>
        private bool CleanDatabase(string dataTable)
        {
            ...
            if (connection != null)
            {
                try
                {
                    ...
                    if (count < 0)
                    {
                        _logger.LogWarning("Impossible to delete old data from {table} at time {time}", dataTable, DateTimeOffset.UtcNow);
                    }
                    else if (count > 0)
                    {
                ...
                {
                    _logger.LogError(ex, "Impossible to clean old data from {table}", dataTable);
                }
            }
            else
            {
